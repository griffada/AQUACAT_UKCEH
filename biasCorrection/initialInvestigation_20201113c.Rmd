...
write_csv(am_summ_obs, "S:/Other Data/BiasCorrection/am_obs.csv")
write_csv(am_sim_obs,  "S:/Other Data/BiasCorrection/am_sim_obs.csv")
```
...
readr::write_csv(x=am_summ_TBC, path="S:/Other Data/BiasCorrection/am_TBC.csv")
readr::write_csv(x=am_summ_FBC, path="S:/Other Data/BiasCorrection/am_FBC.csv")
```


```{r reloader, echo=F}
# This just skirts round having to re-run the above chunk, as it's time-consuming.
if(file.exists("S:/Other Data/BiasCorrection/am_obs.csv")){
  am_summ_obs <- read_csv("S:/Other Data/BiasCorrection/am_obs.csv", 
                          col_types=cols(
                            station = col_double(),
                            year = col_double(),
                            value = col_double()
...
if(file.exists("S:/Other Data/BiasCorrection/am_TBC.csv")){
  am_summ_TBC <- read_csv("S:/Other Data/BiasCorrection/am_TBC.csv",
                          col_types=cols(
                            rcm = col_double(),
                            station = col_double(),
                            year = col_double(),
...
if(file.exists("S:/Other Data/BiasCorrection/am_FBC.csv")){
  am_summ_FBC <- read_csv("S:/Other Data/BiasCorrection/am_FBC.csv",
                          col_types=cols(
                            rcm = col_double(),
                            station = col_double(),
                            year = col_double(),
...
if(file.exists("S:/Other Data/BiasCorrection/am_sim_obs.csv")){
  am_sim_obs <- read_csv("S:/Other Data/BiasCorrection/am_sim_obs.csv",
                          col_types=cols(
                            station = col_double(),
                            year = col_double(),
                            value = col_double()
...
write_csv(am_glo_obs, "S:/Other Data/BiasCorrection/glo_obs.csv")
write_csv(am_glo_simobs, "S:/Other Data/BiasCorrection/glo_simobs.csv")
write_csv(am_glo_TBC, "S:/Other Data/BiasCorrection/glo_TBC.csv")
write_csv(am_glo_FBC, "S:/Other Data/BiasCorrection/glo_FBC.csv")
```

```{r reloader2, echo=F}
# This just skirts round having to re-run the above chunk, as it's time-consuming.
if(file.exists("S:/Other Data/BiasCorrection/glo_obs.csv")){
  am_glo_obs <- read_csv("S:/Other Data/BiasCorrection/glo_obs.csv", 
                          col_types=cols(
                             station = col_double(),
                             loc=col_double(),
                             sca=col_double(),
...
if(file.exists("S:/Other Data/BiasCorrection/glo_TBC.csv")){
  am_glo_TBC <- read_csv("S:/Other Data/BiasCorrection/glo_TBC.csv",
                          col_types=cols(
                             rcm = col_double(),
                             station = col_double(),
                             loc=col_double(),
...
if(file.exists("S:/Other Data/BiasCorrection/glo_FBC.csv")){
  am_glo_FBC <- read_csv("S:/Other Data/BiasCorrection/glo_FBC.csv",
                          col_types=cols(
                             rcm = col_double(),
                             station = col_double(),
                             loc=col_double(),
...
if(file.exists("S:/Other Data/BiasCorrection/glo_simobs.csv")){
  am_glo_simobs <- read_csv("S:/Other Data/BiasCorrection/glo_simobs.csv", 
                          col_types=cols(
                             station = col_double(),
                             loc=col_double(),
                             sca=col_double(),
...
png("./kingston3.png", width=160, height=100, units='mm', res=300, pointsize=11)
...
legend("topleft", legend=c("OBS", "BC", "NBC", "SIM"),
...
png("./eden3.png", width=160, height=100, units='mm', res=300, pointsize=11)
par(mar=c(3,3,1,1), mgp=c(2,1,0), mfrow=c(3,1))
...
png("./kingston4.png", width=160, height=100, units='mm', res=300, pointsize=11)
par(mar=c(3,3,1,1), mgp=c(2,1,0), mfrow=c(3,1))
...
png("./kingston5.png", width=160, height=100, units='mm', res=300, pointsize=11)
par(mar=c(3,3,1,1), mgp=c(2,1,0), mfrow=c(3,1))
...
lightorange <- rgb(222, 145, 44, 0.3*225, maxColorValue=255)
lightpurple <- rgb(136, 42, 219, 0.3*255, maxColorValue=255)
lightgreen <- rgb(37, 110, 22, 255, maxColorValue=255)
...
png("./Allrcms.png", width=170, height=120, units='mm', pointsize=11, res=300)
par(mar=c(3,3,0.1,0.1), mgp=c(2,1,0))

plot(am_obs1$year, am_obs1$value, col=1, lwd=3, type='l', xlab=ifelse(i>10,"Year",""),
     ylab="", ylim=range(am_tbc1$value, am_fbc1$value, am_obs1$value, na.rm=T))
  lines(am_sim1$year, am_sim1$value, col=lightgreen, lwd=3)
for(i in 1:length(rcm_nos)){
  ...
  am_fbc1 <- am_summ_FBC %>%
    dplyr::filter(station==76005 & rcm==rcm_nos[i]) %>%
    select(year, value)
  lines(am_tbc1$year, am_tbc1$value, col=lightorange)
  lines(am_fbc1$year, am_fbc1$value, col=lightpurple)
...
```{r datavis3, fig.width=17, fig.height=12}
...
r2g <- colorRamp(c("red", "white", "darkblue"))
r2g2 <- colorRampPalette(c("red", "white", "darkblue"))(11)

png("./MapQMED.png", width=120, height=120, units="mm", res=300, pointsize=11)
par(mar=c(1,1,3,1), mfrow=c(1,2), mgp=c(2,1,0))
...
png("./Mapsq20.png", width=120, height=120, units="mm", pointsize=10, res=300)
par(mfrow=c(1,2), mar=c(0,0,0,2))
plot(uk_outline, xlim=c(5000, 630000), ylim=c(9000, 950000))
points(posUK$easting, posUK$northing, cex=0.7, col=rgb(r2g(Qpc2), maxColorValue=256))
text(330000, 9000, "With Bias Correction")
#image.plot(Qpc2, add=T, breaks=seq(from=-2,to=2,length.out=12), col=r2g2,
#           legend.only=T)
...
plot(uk_outline, xlim=c(0, 630000), ylim=c(9000, 950000))
points(posUK$easting, posUK$northing, cex=0.7, col=rgb(r2g(Qpc2), maxColorValue=256))
text(330000, 9000, "Without Bias Correction")
image.plot(Qpc2, add=T, breaks=seq(from=-200,to=200,length.out=12), col=r2g2,
           legend.only=T, legend.lab="Percentage change", legend.shrink=c(0.5,1),
           smallplot=c(0.78,0.80, 0.5, 0.98), legend.line=-1.5)
dev.off()
