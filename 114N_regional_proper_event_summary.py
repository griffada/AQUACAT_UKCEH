...
#method="HT" # "EC" OR "OBS", "HT" AS NEEDED
    ...
    summtable_out = pd.DataFrame(columns=["eventNumber", "eventDay", "eventLength",
                                          "area","peakA", "peakAmid", "peakD", "season",
                                          "nclusters","peakyness"])
    ...
    for i in range(NE):
        ...
        amid = min(avec_mid[:,i])
        dvec = min(dvec_all[:,i])
        D = init_table.iloc[ni-1,1:3]
        summtable_out.loc[i] = [ni, D[0], D[1], vvec, avec, amid, dvec, init_table.iloc[ni-1,3], 0, 0]
    ...
    summtable_out.to_csv("".join([outlevel, r'/RCM', rcm, '_', period, subfold,
                      r'/eventSumm_', method, fileinfix,'_RCM',rcm,
                      '_',period, '.csv'])) 
