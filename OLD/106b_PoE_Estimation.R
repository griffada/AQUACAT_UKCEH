...
colnames(partable) <- c("meanint", "threshold", "loc", "sca", "shape", "threshquan")
...
for(h in 1:NH){
  ...
  gpa_poe <- (1 - cdfglo(as.numeric(unlist(obs_events[h,])),
                       vec2par(c(locH,scaleH,shapeH), type='glo')))
  gpa_poe[is.na(gpa_poe)] <- 0
  ...
  if(any(gpa_poe < 1e-3)){
    ...
    print(paste("obs beyond 1000 yr", paste(obs_events[h, gpa_poe < 1e-3],
                                            collapse=" ")))
