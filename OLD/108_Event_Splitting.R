...
obs_dpe <- as.data.frame(readr::read_csv(paste0(data_wd, subfold, "eventdpe_OBS_", thresh1,
                                       "_", ws1, "_RCM", RCM, suffix, ".csv"),
                           col_types=cols(.default = col_double()))[,-(1:4)])
  
obs_ape <- as.data.frame(readr::read_csv(paste0(data_wd, subfold, "eventape_OBS_", thresh1,
                                       "_", ws1, "_RCM", RCM, suffix, ".csv"),
                           col_types=cols(.default = col_double()))[,-(1:4)])
...
for(REG in regions){
  ...
  obs_dpe_region <- cbind(rn[r1,], obs_dpe_region)
  
  obs_events_region <- obs_events_region[,KEEP]
  obs_events_region <- cbind(rn[r1,], obs_events_region)
  
  
  obs_ape_region <- obs_ape_region[,KEEP]
  obs_ape_region <- cbind(rn[r1,], obs_ape_region)
  ...
  write_csv(obs_events_region, path=paste0(data_wd, subfold, "/",
                                      REG, "/eventflow_OBS_region_",
                                      REG,"_RCM", 
                                      RCM, suffix,".csv"))
  
  write_csv(obs_dpe_region, path=paste0(data_wd, subfold,  "/",
                                      REG, "/eventdpe_OBS_region_",
                                      REG,"_RCM", 
                                      RCM, suffix,".csv"))
  
  write_csv(obs_ape_region, path=paste0(data_wd, subfold,  "/",
                                      REG, "/eventape_OBS_region_",
                                      REG,"_RCM", 
                                      RCM, suffix,".csv"))
}
...
#### EC SPLITTING ####---------------------------------------------------------
obs_events <- readr::read_csv(paste0(data_wd, subfold, "eventflow_EC_", thresh1,
                                     "_", ws1, "_RCM", RCM, suffix, ".csv"),
                              col_types=cols(.default=col_double()))[,-(1:4)]

obs_dpe <- readr::read_csv(paste0(data_wd, subfold, "eventdpe_EC_", thresh1,
                                       "_", ws1, "_RCM", RCM, suffix, ".csv"),
                           col_types=cols(.default = col_double()))[,-(1:4)]
  
obs_ape <- readr::read_csv(paste0(data_wd, subfold, "eventape_EC_", thresh1,
                                       "_", ws1, "_RCM", RCM, suffix, ".csv"),
                           col_types=cols(.default = col_double()))[,-(1:4)]
