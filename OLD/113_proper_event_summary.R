...
# if(file.exists(paste0(data_wd,subfold, "eventSumm_OBS_",
...
obs_event_flow  <- as.data.frame(readr::read_csv(paste0(data_wd,subfold, "eventflow_OBS_",
                                      thresh1,"_", ws1,"_RCM", RCM, suffix, ".csv"),
                               col_types=cols(.default = col_double()))[,-(1:4)])

obs_event_ape  <- as.data.frame(readr::read_csv(paste0(data_wd,subfold, "eventape_OBS_",
                                      thresh1,"_", ws1,"_RCM", RCM, suffix, ".csv"),
                               col_types=cols(.default = col_double()))[,-(1:4)])

obs_event_dpe <- as.data.frame(readr::read_csv(paste0(data_wd,subfold, "eventdpe_OBS_",
                                        thresh1,"_", ws1,"_RCM", RCM, suffix, ".csv"),
                                 col_types=cols(.default = col_double()))[,-(1:4)])
...
for(i in 1:(ncol(obs_event_flow))){
  ...
  eventSummaryFrame[i,] <- list(i, D, L, sum(vvec > threshMat[,jT]),
