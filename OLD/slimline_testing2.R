...
saveRDS(MODELS, file=paste0(wd_id, "slimline/MODELS.rds"))
...
k <- 1
step3_test1 <- mexDependence_slim(dqu=0.7, mth=step1_test1$mth, which=k, marginsTransformed=TRANSFORMED)
str(step3_test1, max.level=2)

k <- 2
step3_test2 <- mexDependence_slim(dqu=0.7, mth=step1_test1$mth, which=k, marginsTransformed=TRANSFORMED)
...
step4_test1 <- lapply(1:NREG,
                      function(k){if(k %% 50 == 0)print(paste0("Dependence at", 100*k/NREG, "%"))
                        try(mexDependence_slim(dqu=dqu, mth=step1_test1$mth, which=k))})
...
# 
nSample <- 50  # still a very low number of events.
d <- NREG
mult <- 10
k <- 1
