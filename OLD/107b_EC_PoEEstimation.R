...
eventDF <- readr::read_csv(paste0(data_wd,subfold, "NewEventEC_",thresh1,"_", ws1,
                                  "_RCM", RCM, suffix, ".csv"),
                           col_types=cols(
                             .default = col_double()
                           ))
...
for(h in 1:nrow(eventDF)){
  ...
  rps <- as.numeric(pevd(eventDF[h,-(1:4)], scale=partable$scale[h],
                         shape=partable$shape[h], threshold = thr, type='GP'))
...
write_csv(data.frame(rarityDF), 
          paste0(data_wd,subfold, "OBSraritydf_", thresh1, "_", ws1, "_RCM", RCM, suffix, ".csv"))
