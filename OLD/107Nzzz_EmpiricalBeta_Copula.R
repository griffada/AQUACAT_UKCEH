...
savepath <- paste0(data_wd, subfold, "eventECX_",
...
flow_glo <- function(v, params, eventthresh, OBS){
 ...
 flow <- rep(0, length(v))
 up <- v < 1-eventthresh
 setpars <- pargpa(lmoms(OBS[OBS>params$threshold]), xi=params$threshold)
...
betacop <- function(NE, NH, dM, rankevents, thresholds=(2/360), mincov=0.001){
...
# 
newEventDraw <- c()
# newEventDraw <- sample.int(NE, M, replace=T)

obs_slice <- ncvar_get(obs_events, "flow") #NH x NE

obs_thresh <- 1 - sapply(1:NH, function(i){ecdf(obs_slice[i,])(threshMat[i,2])})

rank_events <- t(apply(-obs_slice, 1, rank, ties.method = "random")) #NHxNE

rank_events0 <- t(apply(-obs_slice[1:9,1:15], 1, rank, ties.method="random"))


betacop(NE=15, NH=9, dM=2, rank_events0, thresholds=obs_thresh[1:9],
       mincov=0.001)
...
Vnew <- foreach(m = 1:(M/dM) , .combine = cbind) %dopar% {
  if(m %% 200 == 0){print(paste0(m*dM,"/",M))}
...
#evStart <- ifelse(ec_events$var$eventNo$size > 1, ec_events$var$eventNo$size+1, 1)
# V to flow
#flow_frame <- matrix(NA, nrow=NH, ncol=M)
for(h in 1:NH){
  if(h < 10 | h %% 1000 == 0){print(h)}
  obs_location <- obs_slice[h,]
  o <- try(
    flow_glo(Vnew[h, ], params = partable_pres[h, ],
  ...
  if(inherits(o, c("Error", "try-error"))){
   ...
   print(o)
    ...
    #flow_frame[h,] <- unlist(o)
   ncvar_put(ec_events, "flow", unlist(o), start=c(h,1), count=c(1,M))
  }
}
