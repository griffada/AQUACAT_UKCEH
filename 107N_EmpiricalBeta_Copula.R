...
flow_glo <- function(v, params, eventthresh, OBS){
    ...
    if (sum(OBS > params$threshold) < 12){
      up[] <- FALSE
    } else if (!are.lmom.valid(lmoms(OBS[OBS>params$threshold]))){
      up[] <- FALSE
    } else if (!are.pargpa.valid(pargpa(lmoms(OBS[OBS>params$threshold]),
                        xi=params$threshold))){
      up[] <- FALSE  
    }
    if(any(up)){
...
Vnew1 <- foreach(m = 1:(M/dM)) %dopar% {
  ...
  B <- betacop(NE, NH, dM, rank_events, thresholds=obs_thresh, mincov=0.001)
