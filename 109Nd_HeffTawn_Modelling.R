...
if(settings$HTflow){
 print("HT flow already exists. Stopping 109N.") 
}else{
REG <- "NW"
...
### STEP 3 ### DEPT STRUCT ### ---------------------
if(settings$HTstructure &
   file.exists(paste0(data_wd, subfold, REG,  "/depStruct", REG, "_",
...
en <- max(which(ncvar_get(ht_events, "eventNo") > 0)+1, 1)
ncvar_put(ht_events, "eventNo", W, start=en, count=min(nSample, dS))
ncvar_put(ht_events, "flow", MCS, start=c(1,en), count=c(-1,min(nSample, dS)))
...
if(nSample > dS){
    ...
    for(i in seq_len(length(sampleStep)-1)){
      ...
      en <- max(which(ncvar_get(ht_events, "eventNo") > 0), 1)
      ncvar_put(ht_events, "eventNo", W, start=en, count=length(W))
      ncvar_put(ht_events, "flow", MCS, start=c(1,en), count=c(-1,ncol(MCS)))
