...
QPOT5 = pd.read_csv(
    mainfolder + "/Data/qPOT" + events_per_year + "_sans_mask.csv", header=None)

extremes = pd.read_csv(
    "{}/Data/extremedates_{}inun_{}epy.csv".format(
        mainfolder, int(inun_thres*100), events_per_year), header=True)
...
im = ax.imshow(flow_wide.mask, cmap=cmap, vmin=0.1)
