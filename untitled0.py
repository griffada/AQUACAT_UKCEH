...
ncout = Dataset('/prj/aquacat/CodeABG/InterimData/trial0.nc', 'w', format='NETCDF4')
...
time.long_name = 'Time'
time.units = 'days since 1961-1-1 0:0:0'
...
vout = ncout.createVariable('dmflow', np.float32, ('Northing', 'Easting', 'Time'))
...
for i in range(len(A)-1):
    vout[:,:,A[i]:A[i+1]] = np.moveaxis(ncin.variables['dmflow'][A[i]:A[i+1],:,:],0,2)
    print(i)
